...
(() => {
    ...
    function bindUIEvents() {
        ...
        if (deduplicateBtn) {
            deduplicateBtn.addEventListener('click', deduplicateKeys);
        }
        ...
        if (cancelBtn) {
            cancelBtn.addEventListener('click', stopTest);
        }

        const resultsDiv = document.getElementById('results');
        if (resultsDiv) {
            resultsDiv.addEventListener('click', (event) => {
                const copyBtn = event.target.closest('#copyAllBtn');
                if (copyBtn) {
     ...
     * @returns {Promise<Object|null>} - 测试结果（取消时返回 null）
     */
    async function testApiKey(apiUrl, apiKey, modelName, abortSignal) {
    ...
    function displayResults() {
        ...
        // 显示统计
        const totalInvalid = Object.values(testResults.invalid).reduce(
    ...
    `;
        ...
        Object.entries(testResults.invalid).forEach(([category, keys]) => {
            ...
            header.dataset.target = sectionId;
            ...
            if (items.length > visibleCount) {
                ...
                if (page > 0) {
                    ...
                    prevBtn.style.cssText =
                        'padding: 8px 16px; background: #2196F3; color: white; border: none; border-radius: 4px; cursor: pointer;';
    ...
    function toggleSection(bodyId) {
        ...
        const iconId = bodyId.endsWith('Body') ? bodyId.slice(0, -4) + 'Icon' : bodyId + '_icon';
